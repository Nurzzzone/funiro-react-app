...
function Cart() {
  const [cartItemsQuantity, setCartItemsQuantity] = React.useState(0);
  ...
  return (
    <div ref={cartRef}>
      <div onClick={toggleCartOpen} className="btn header__profile-button header__profile-button--cart">
        <svg 
          width="24" 
          height="24" 
          viewBox="0 0 24 24" 
        ...
        <span onChange={itemsQuantity} className="cart-counter">{}</span>
      </div>
      {
        cartOpen && <CartPopup />
      }
